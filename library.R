...
colors_panel2 <- c("blue", "brown4", "darkmagenta")
...
learning_plot <- function(
    ...
    if (is.null(colors)) {
        panel <- panel +
            geom_line(data=annotation_df, aes(x=x, y=PI, group=group__), color="#ff6000", size=2) +
...
sleep_plot <- function(dt, point_size=3, line_size=2, color_by_group=TRUE, colors=NULL) {
    ...
    if (color_by_group) {
        ...
        sleep_plot <- ggplot(data=dt[variable=="asleep",], aes(x=t, y=mu, color=group__, group=group__)) +
            ...
            scale_x_hours(breaks=behavr::hours(seq(4, 30, 2)), name="ZT") +
            scale_y_continuous(name = "% 30 min asleep", breaks = seq(0, 1, 0.2), labels=seq(0, 1, 0.2)*100, limits=c(0, 1)) +
        ...
        sleep_plot <- ggplot() +
            ...
            scale_x_hours(breaks=behavr::hours(seq(4, 30, 2))) +
            scale_y_continuous(
                name = "% 30 min asleep", breaks = seq(0, 1, 0.2), labels=seq(0, 1, 0.2)*100,
